...
// Actions
export type AppendLogAction = {
  type: "APPEND_LOG";
  payload: { date: string; msg: string; error: boolean };
};
...
export function executeQuery() {
  return async (dispatch: Dispatch<object>, getState: () => State) => {
    const url = `/api?query=${encodeURIComponent(getState().query.sql!)}`;

    dispatch(appendLog(`sending SQL query to server: ${url}`));
    ...
    try {
      ...
      if (response.ok) {
        dispatch({
          type: "EXECUTE_QUERY",
          payload: worker("executeQuery", text).catch((error: any) => {
            dispatch(appendLog(error, true));
          }),
        });
      } else {
        dispatch(appendLog(`received error ${response.status}: "${text}"`));
      }
    } catch (e) {
      dispatch(appendLog(e.message));
    }
  };
}
...
export type ChangeQueryAction = { type: "CHANGE_QUERY"; payload: string };
...
export function printResult(result: List<string>) {
  return { type: "PRINT_RESULT", result };
}
...
export const loadFiles = (fileHandles: FileList) => (
  ...
  for (let i = 0; i < fileHandles.length; i++) {
    ...
    else canLoad.push(fileHandles[i]);
  }

  if (canNotLoad.length > 0)
    dispatch(
      notify(
        `file${canNotLoad.length > 1 ? "s" : ""}: ${canNotLoad} already loaded`
      )
    );
  ...
  dispatch({
    type: "LOAD_FILES",
    async payload() {
      return worker("loadFiles", fileHandles).catch((error: any) => {
        dispatch(appendLog(error, true));
      });
    },
  });
...
export const removeFile: (fileName: string) => RemoveFileAction = (
  fileName
) => ({ type: "REMOVE_FILE", fileName });

type EngineErrorAction = { type: "ENGINE_ERROR"; msg: string };
export const engineError: (msg: string) => EngineErrorAction = (msg) => ({
  type: "ENGINE_ERROR",
  msg,
});
...
export function reducer(state = initialState, action: PossibleAction): State {
  switch (action.type) {
    ...
    case "REMOVE_FILE":
      return {
        ...state,
        filePreviews: state.filePreviews.filter(
    ...
    case "PRINT_RESULT":
      const splitted = action.result.flatMap((fragment) =>
        fragment.split("\n")
      );
