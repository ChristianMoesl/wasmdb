...
export type CmdLoadFiles = {
  type: "request";
  uuid: string;
  command: "loadFiles";
  payload: File[];
};
export type CmdFilesLoaded = {
  type: "response";
  uuid: string;
  answers: "loadFiles";
  payload: FilePreview[];
};
export type CmdExecuteQuery = {
  type: "request";
  uuid: string;
  command: "executeQuery";
  payload: string;
};
export type CmdFinishedExec = {
  type: "response";
  uuid: string;
  answers: "executeQuery";
  payload: null;
};
export type CmdPrint = {
  type: "request";
  uuid: string;
  command: "print";
  payload: string;
...
export type CmdError = {
  type: "error";
  uuid: string;
  answers: "loadFiles" | "executeQuery";
  payload: string;
};
...
module.exports = (self: any) => {
  async function handle(request: WorkerRequest) {
    try {
      switch (request.command) {
        case "loadFiles":
          const filesLoaded: FileContent[] = await loadFiles(
            request.payload as File[]
          );
          ...
          postResponse({
            ...
            uuid: request.uuid,
            answers: "loadFiles",
            payload: filePreviews,
          });
          break;
        case "executeQuery":
          await executeQuery(request.payload);
  ...
  async function executeQuery(wasm: string) {
    try {
      const binary = Buffer.from(wasm, "base64");
    ...
    return new Promise<FileContent[]>((resolve, reject) => {
      ...
      for (const file of fileHandles) {
        ...
        reader.onload = (event) => {
          ...
          if (loadedFiles.length === fileHandles.length) resolve(loadedFiles);
        };
