...
export function createWasmWorker(
  ...
  function handle(response: WorkerResponse) {
    switch (response.type) {
      case "request":
        ...
        break;
      case "error":
        rejectResponse(response.uuid, response.payload);
        break;
      case "response":
        resolveResponse(response.uuid, response.payload);
        break;
    }
  }
  ...
  return async function sendRequest<T extends WorkerResponse>(
    command: string,
    payload?: any
  ): Promise<T> {
    switch (command) {
      ...
      case "terminateExecution":
        ...
        commandRejectors.forEach((reject) =>
          reject(new Error("WASM worker terminated"))
        );
        break;
      default:
        break;
